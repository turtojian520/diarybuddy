...
export function MobileQuickInputBar({ onOpen, nextIndex }: Props) {
  ...
  return (
    <button
      ...
      onClick={onOpen}
      aria-label="打开写碎片输入"
      className="fixed inset-x-3 bottom-[calc(var(--db-mobile-nav-height)+env(safe-area-inset-bottom,0px)+0.625rem)] z-30 flex min-h-14 items-center gap-3 rounded-full border border-[var(--db-border)] bg-[var(--db-bg)] px-4 py-2 text-left shadow-[0_8px_24px_rgba(43,42,39,0.12)] md:hidden"
    >
